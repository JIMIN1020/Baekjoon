...
### ✔️ 풀이
문자열을 string으로 받아온 후, 6자리 씩 A~H에 해당하는 값과 한자리 한자리 비교함.

일치하거나 1자리만 다른 경우, 해당 문자를 result 문자열에 저장.

판단이 끝난 6자리는 (C++)`str.erase(0, 6)` (Java)`str.substring(6)` 으로 제거한 뒤 다음 문자 검사.
